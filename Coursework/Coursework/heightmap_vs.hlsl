...
//#define DISPLACE 0.14f
    ...
    north.x = input.tex.x;
    north.y = input.tex.y + OFFSET;

    east.x = input.tex.x + OFFSET;
    east.y = input.tex.y;

    south.x = input.tex.x;
    south.y = input.tex.y - OFFSET;

    west.x = input.tex.x - OFFSET;
    west.y = input.tex.y;
	...
	// vectors
    tc = normalize(top - centre);
    rc = normalize(right - centre);
    bc = normalize(bottom - centre);
    lc = normalize(left - centre);
